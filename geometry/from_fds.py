...
def xbs_to_ob(
    xbs, context, ob=None, bf_xb="NONE", name="xbs_to_ob", update_center=True
) -> "Mesh":
    ...
    if ob:
        ...
        ob = utils.get_new_object(
            context, context.scene, name, me
        )  # no ob, get a new one with proper mesh
    if update_center:
        utils.set_balanced_center_position(context, ob)
    # ob.bf_xb = bf_xb # FIXME
    return ob
...
def xyzs_to_ob(
    xyzs, context, ob=None, bf_xyz="NONE", name="xyzs_to_ob", update_center=True
) -> "Mesh":
    """Transform geometry in FDS notation to Blender object."""
    # Choose bf_xyz
    if bf_xyz == "NONE":
...
choose_from_pbs = {"NONE": none_to_mesh, "PLANES": pbs_planes_to_mesh}


def pbs_to_ob(
    pbs, context, ob=None, bf_pb="NONE", name="pbs_to_ob", update_center=True
) -> "Mesh":
    """Transform geometry in FDS notation to Blender object."""
    # Choose bf_pb
    if bf_pb == "NONE":
